# -*- coding: utf-8 -*-
"""
Created on Tue Apr  8 21:09:34 2025
...
def check_for_matches(query_embedding, metadatas, embeddings, username, mode):
    ...
    logger.debug(f"Metadatas count: {len(metadatas)}, Embeddings count: {len(embeddings)}")
    
    if not embeddings or len(metadatas) != len(embeddings):
        logger.warning("No valid embeddings to match against or metadata-embedding mismatch")
        return "No image or caption matches found.", []
